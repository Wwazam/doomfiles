...
(defun todo-template (letter name)
  (let* ((downcase-name (downcase name)))
    `(,letter ,name entry (file "inbox.org") (format "* TODO %%? :%s:\n" downcase-name))))
(setq org-capture-templates
      `(("t" "Todo")
        ...
        ("p" "Project")
        ("pt" "Todo" entry (file+headline ,(org-roam-db ) "Tasks"))
        ))
...
;; (defun capture-project-name ()
;;   (let ((name (read-string "Project: "))
;;         (time (format-time-string "%Y-%m-%d")))
;;     (format "%s_%s.org" time name)))

;; (defun visit-file ()
;;   (interactive)
;;   (let ((path (capture-project-name)))
;;     (set-buffer (org-capture-target-buffer path))
;;     ;; (goto-char (point-max))
;;     ;; (org-store-link 1)
;;     ))
